        ...
        <div className=" col-span-2 justify-self-start flex gap-2 md:gap-4">
            <Link href={`/product/${item.id}`}>
               <div className=" relative aspect-square">
                <Image src={item.selectedImg.image} width={70} height={100} alt={item.name} fill className=' object-contain' />
                </div> 
            </Link>
            <div className=" flex flex-col justify-between">
                ...
                <div className=" w-[70px]">
                    <button className='text-slate-50 underline' onClick={() => handleRemoveProductFromCart(item)}>
                        Remove
                    </button>
                </div>
            </div>
